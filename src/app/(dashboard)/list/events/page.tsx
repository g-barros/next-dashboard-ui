...
export default async function EventsListPage({searchParams}: { searchParams: { [key: string]: string | undefined } }) {
  ...
  const renderRow = (item: EventList) => (
    <tr key={item.id} className="border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-lamaPurpleLight">
      ...
      <td className="hidden md:table-cell">{item.endTime.toLocaleTimeString("pt-BR", {
        hour:"2-digit",
        minute:"2-digit",
        hour12: false,
      })}</td>
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 mt-0 m-4">
      ...
      <Table columns={columns} renderRow={renderRow} data={eventsList} />
      <Pagination page={p} count={eventsCount} />
    </div>
  )
}