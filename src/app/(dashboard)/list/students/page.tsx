...
export default async function StudentsListPage({searchParams}: { searchParams: { [key: string]: string | undefined } }) {
  ...
  const [studentsList, studentsCount] = await prisma.$transaction([
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 mt-0 m-4">
      ...
      <Table columns={columns} renderRow={renderRow} data={studentsList} />
      <Pagination count={studentsCount} page={p} />
    </div>
  )
}