...
export default async function ResultsListPage({searchParams}: { searchParams: { [key: string]: string | undefined } }) {
  ...
  const [resultsListRes, resultsCount] = await prisma.$transaction([
  ...
  const resultsList = resultsListRes.map(item => {
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 mt-0 m-4">
      ...
      <Table columns={columns} renderRow={renderRow} data={resultsList} />
